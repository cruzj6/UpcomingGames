...
module.exports = {
    ...
    getUserTrackedGameData: function getUserTrackedGameData(userId, handleTrackedGameData) {
            ...
            if (ids && ids.length > 0) {
                ...
                for (var i = 0; i < ids.length; i++) {
                    ...
                    gameAPI.getDataForGameById(ids[i].gameid, function (gameData) {
                        ...
                        if (returnGameData.length == successfulGets && attempts == ids.length) handleTrackedGameData(returnGameData);
                    });
                }
            } else {
    ...
    addTrackedGameId: function addTrackedGameId(gameId, userid, doneCallback) {
        dbm.addGameIDToUser(gameId, userid, doneCallback);
    },

    //Use database manager module to remove a tracked game
    removeTrackedGameId: function removeTrackedGameId(gameId, userId, doneCallback) {
        dbm.removeGameIDFromUser(gameId, userId, doneCallback);
    },

    getSteamFriendsTrackedGames: function getSteamFriendsTrackedGames(usersteamid, handleFriendsTrackedGames) {
...
//# sourceMappingURL=userdataprocessor-compiled.js.map